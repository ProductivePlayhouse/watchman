...
export const getSDNAlts = async (sdnId) =>
  (await fetch(pathname(url) + `/ofac/sdn/${sdnId}/alts`)).json();
export const getSDNAddresses = async (sdnId) =>
  (await fetch(pathname(url) + `/ofac/sdn/${sdnId}/addresses`)).json();
export const getSDNTypes = async (qs) => (await fetch(pathname(url) + `/ui/values/sdnType`)).json();
export const getOFACPrograms = async (qs) =>
  (await fetch(pathname(url) + `/ui/values/ofacProgram`)).json();
